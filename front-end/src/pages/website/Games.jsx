...
export default function Games() {
	...
	useEffect(() => {
		if (search === "") {
			...
			const puzzles = games.filter((game) =>
		...
		<div className="   flex">
			...
			<div className="max-w-2xl w-full px-4 sm:py-24 sm:px-6 lg:max-w-7xl lg:px-8">
				<div className="grid grid-cols-1 gap-y-10 gap-x-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 xl:gap-x-8">
					{items.length === 0 ? (
						<p className="text-3xl text-center">No Games</p>
					) : (
						items?.map((game, i) => {
							return (
								<Link to={`/games/${game.guid}`} key={i} className="group">
									...
									<h3 className="mt-4 text-sm text-gray-700 dark:text-cream">
										{game.title}
									</h3>
